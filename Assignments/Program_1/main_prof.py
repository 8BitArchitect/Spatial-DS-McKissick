...
class DrawGeoJson(object):
    ...
    def add_polygon(self,poly,id=None):
        """
        Add a polygon to local dictionary to be drawn later
        Args:
            poly (list): list of lat/lons
            id (string): identifier of country or state
        Returns:
            None
        """
        if id is not None:
    ...
    def adjust_poly_dictionary(self):
        """
        Take the 'latlon' lists in the dictionary, and convert them to 'xy' saving
        them like the following:
            {
                'TX': {
                    'latlon': [
                        [(-106.633301, 31.952162),
                            (-106.303711, 31.597252),
                            (-106.633301, 31.952162)
                        ]
                    ],
                    'xy': [
                        **** add converted points here *****
                    ]
                }
        Args:
            None
        Returns:
            None
        """
        for country,polys in self.poly_dict.items():
            new_polys = []
            for poly in polys['latlon']:
                new_poly = []
                for p in poly:
                    x,y = p
                    new_poly.append(self.convertGeoToPixel(x,y))
                new_polys.append(new_poly)
            self.poly_dict[country]['xy'] = new_polys
    ...
    def draw_polygons(self):
        ...
        for country,data in self.poly_dict.items():
            for poly in data['xy']:    
                pygame.draw.polygon(self.screen, data['color'], poly, 0)
    ...
    def find_intercepting_country(self,x,y):
        for key,polygons in gd.poly_dict.items():
            for poly in polygons['xy']:
                res = point_inside_polygon(x,y,poly)
                if res:
                    return key
        return None

    def highlight_country(self,key):
        for poly in self.poly_dict[key]['xy']:    
            pygame.draw.polygon(self.screen, self.colors.get_rgb('black'), poly, 3)
        bounds = self.get_max_coords(key)
        self.print_bounding_box(bounds)
        
    def get_max_coords(self,key):
        lats = []
        lons = []
        for poly in self.poly_dict[key]['xy']:    
            for p in poly:
        ...
        return {'maxX':max(lons),'maxY':max(lats),'minX':min(lons),'minY':min(lats)}

    def print_bounding_box(self,b):
        poly = []
        poly.append((b['minX'],b['minY']))
        poly.append((b['maxX'],b['minY']))
        poly.append((b['maxX'],b['maxY']))
        poly.append((b['minX'],b['maxY']))
        poly.append((b['minX'],b['minY']))
        pygame.draw.polygon(self.screen, self.colors.get_rgb('red'), poly, 4)
...
if __name__ == '__main__':
    ...
    #df.add_polygons(['TX','Spain','France','Belgium','Italy','Ireland','Scotland','Greece','Germany','Egypt','Morocco','India','Maine','California','Florida',"UZB","Albania",'Switzerland'])
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                print(event.pos)
                x,y = event.pos
                key = gd.find_intercepting_country(x,y)
                if key:
